...
int h2(int x, int m)
{
    ...
    return 2*(100003-(x % 100003))+1;
}

//int h(int x, int m, int i)
//{
//    ilosc = ilosc + 1;
//    return(x % m + 5*i*i-2*i) % m;
//}
...
int main()
{
    ...
    for (float j = 0; j < 10; j++)
    {
        ...
        for (int i = zapelnienie; i < zapelnienie + 10000; i++)
        {
            hash_al_wstaw(tab, m, dane[i]);
        }
        ...
        std::cerr << czas.count() / 10000.0 << " " << ilosc / 10000.0 << std::endl;
    ...
    for (float j = 0; j < 10; j++)
    {
        ...
        for (int i = zapelnienie; i < zapelnienie + 10000; i++)
        {
            hash_al_szukaj(tab, m, dane[losowa_liczba(0, zapelnienie)]);
        }
        //tutaj mierzona operacja
        //...
        //koniec mierzenia
